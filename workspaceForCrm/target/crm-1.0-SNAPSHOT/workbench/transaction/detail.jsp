...
<%@ page import="com.study.com.settings.domain.DicValue" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.study.com.settings.domain.Tran" %>
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//
			if ("0".equals(currentPossibility)){//1
				for (int i = 0;i < dicValueList.size();i ++){//2
					DicValue dv = dicValueList.get(i);
					String listStage = dv.getValue();
					String listPossibility = pmap.get(listStage);
					if ("0".equals(listPossibility)){//为0，就是黑红叉//3
						if (currentPossibility.equals(listPossibility)){//4
							//红叉
		%>
		...
		<%
			    ...
			    int point = 0;
				for (int i = 0;i < dicValueList.size();i ++){//8
					String stage = dicValueList.get(i).getValue();
					//String Possibility = pmap.get(Stage);
					if (stage.equals(currentStage)){//9
						point = i;
						break;
					}
				}
				for (int i = 0;i < dicValueList.size();i ++) {//10

				    DicValue dv = dicValueList.get(i);
					String listStage = dv.getValue();
					String listPossibility = pmap.get(listStage);
					//如果阶段为0
					if ("0".equals(listPossibility)){//11
						//黑茶
		%>
		      <span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')"
			  class="glyphicon glyphicon-remove mystage"
